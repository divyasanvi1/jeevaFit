...
const sendSOS = async (req, res) => {
  try {
    ...
    if (!user.emergencyContactEmail || !user.emergencyContactPhone) {
      return res.status(400).json({ error: 'User does not have emergency contact details' });
    }
    ...
    console.log(`Emergency alert email sent for user ${userId} at location ${location.lat}, ${location.lng}`);

    res.status(200).json({ message: 'SOS alert sent successfully, email dispatched' });
