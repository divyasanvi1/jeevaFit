...
const SignUpPage = () => {
  ...
  const handleSignUp = async (e) => {
    ...
    if (!name || !email || !password || !age || !weight || !height || !emergencyContactEmail || !emergencyContactPhone || !bloodGroup) {
    ...
    if (!validatePassword(password)) {
      setError('Password must be at least 8 characters long and include a mix of letters and numbers.');
      return;
    }
...
<select
    ...
    required
  >
    <option value="" disabled>{t('signup.bloodGroup')}</option> {/* Placeholder */}
  ...
  <div className='w-full flex flex-row items-center justify-between mt-4'>
    ...
    {t('signup.acknowledgement.text')}{' '} <span className='font-bold text-[#03045E]/70 underline'>{t('signup.acknowledgement.acknowledge')}</span> {t('signup.acknowledgement.continue')}{' '} <span className='font-bold text-[#03045E]/70 underline'>{t('signup.acknowledgement.privacy')}</span> {t('signup.acknowledgement.and')}{' '} <span className='font-bold text-[#03045E]/70 underline'>{t('signup.acknowledgement.terms')}</span>. 
    </p>
  </div>
